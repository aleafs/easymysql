/* vim: set expandtab tabstop=2 shiftwidth=2 foldmethod=marker: */

"use strict";

var util = require('util');
var mysql = require('mysql');
var EventEmitter = require('events').EventEmitter;

/**
 * @ Connection
 */
var Connection = function (options) {

  EventEmitter.call(this);

  /**
   * 0 : 未连接
   * 1 : 已连接
   * -1: 准备断开
   */
  this._flag = 0;
  this._conn = mysql.createConnection(options);
  this._name = util.format('%s@%s:%d', options.user, options.host, options.port);

  this.connect(options);
};
util.inherits(Connection, EventEmitter);

Connection.prototype._isalive = function () {
  if (this._flag < 1) {
    return false;
  }

  /**
   * @ close the socket
   */
  if (!this._conn || !this._conn._socket || !this._conn._socket._handle) {
    this._flag = 0;
    this._conn._socket.destroy();
    this.emit('close', 'Lo');
    return false;
  }

  return true;
};

Connection.prototype.connect = function () {

  if (this._isalive()) {
    return true;
  }

  var _self = this;

  /**
   * @ 连接超时
   */
  var tmout = 100;
  var timer = setTimeout(function () {
    // XXX: this make "Aborted_connects" ++ in mysql server 
    _self._conn._socket.end();
    _self.emit('error', new Error('ConnectTimeout'));
  }, tmout);

  _self._conn.removeAllListeners();
  _self._conn.on('error', function (error) {
    if (_self._flag < 0 || !error.fatal) {
      return;
    }
    _self.emit('close', _self._error(error));
  });

  _self._conn.connect(function (e) {
    clearTimeout(timer);
    timer = null;

    if (!e) {
      _self._flag = 1;
    } else {
      _self._flag = 0;
      process.nextTick(function () {
        _self.emit('error', _self._error(e));
      });
    }
  });
};

Connection.prototype._error = function (name, msg) {
  var e;
  if (name instanceof Error) {
    e = name;
    e.name = (e.name && 'Error' !== e.name) ? e.name : 'MysqlError';
  } else {
    e = new Error(msg || name);
    e.name = name;
  }
  e.message = util.format('%s (%s)', e.message, this._name);
  return e;
};

Connection.prototype.clone = function () {
  return new Connection(this._conn.config);
};

Connection.prototype.close = function (callback) {

  callback = callback || function () {};

  if (!this._isalive()) {
    return callback();
  }
  var _self = this;

  _self._flag = -1;
  _self._conn.end(function (error) {
    _self.emit('close');
    callback(error);
  });
};

Connection.prototype.query = function (sql, timeout, callback) {

  if (this._flag > 0 && !this._isalive()) {
    callback(this._error('ConnectionLost', 'Connect to mysql server lost.'));
    return;
  }

  if (!timeout || timeout < 0) {
    this._conn.query(sql, callback);
    return;
  }

  var _self = this;
  var tmout = setTimeout(function () {
    callback(_self._error('QueryTimeout', 'Mysql query timeout after ' + timeout + ' ms'));
    callback = function (e, r) {
      _self.emit('timeout', e, r, sql);
    };
  }, timeout);
  _self._conn.query(sql, function (error, res) {
    clearTimeout(tmout);
    tmout = null;
    callback(error ? _self._error(error) : null, res);
  });
};

exports.create = function (options) {
  return new Connection(options);
};

